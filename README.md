<div align="center">

# 👋 Hi, I'm Ayush Munot

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=9745F5&center=true&vCenter=true&width=650&lines=Graduated+From+%40+IIT+Kharagpur;AI+Researcher+%7C+ML+Engineer;Building+Agentic+%26+Multimodal+AI+Systems)](https://git.io/typing-svg)

<p>
  Graduated with B.Tech in Mechanical Engineering, Master's in Financial Engineering and CS Minor from IIT Kharagpur in 2026<br/>
  Interests: <strong>LLM Reasoning · Agentic AI · Vision-Language Models · AI Safety</strong>
</p>

<!-- [![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/ayush-munot-5b4963223) // -->
<!-- [![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:munotayush6@gmail.com) -->

<a href="https://drive.google.com/file/d/1mJ83_EMGQCLrrXrqiMvFcBMst83HloCc/view?usp=sharing">
    <img src="https://img.shields.io/badge/PDF-CV-red?style=flat-square&logo=adobe">
</a>  
<a href="https://linkedin.com/in/ayush-munot-5b4963223">
    <img src="https://img.shields.io/badge/-Linkedin-blue?style=flat-square&logo=linkedin">
</a>
<a href="mailto:munotayush6@gmail.com">
    <img src="https://img.shields.io/badge/-Email-red?style=flat-square&logo=gmail&logoColor=white">
</a>
<a href='https://scholar.google.com/citations?user=-86DdywAAAAJ&hl=en&authuser=1&oi=sra' target="_blank">
    <img alt='GoogleScholar' src='https://img.shields.io/badge/Scholar-100000?style=flat&logo=GoogleScholar&logoColor=white&&color=0181FF'>
</a>
<a href="https://ayush1298.github.io/">
    <img src="https://img.shields.io/badge/Website-181717?style=flat-square&logo=firefoxbrowser&logoColor=white">
</a>


🏅 **Core Contributor** to [MTEB](https://github.com/embeddings-benchmark/mteb) 

</div>

---

## 🔬 Research Interests

- **Vision–Language Models** — reasoning, grounding, and multimodal alignment
- **Agentic AI & LLM Reasoning** — planning, tool-use, and evaluation of agentic systems
- **AI Safety & Robustness** — multilingual safety, failure modes, and interpretability
- **Explainable AI** — attribution methods, CAM-based analysis of deep models

---

## 🛠️ Tech Stack

<div align="center">

**Languages**

<img src="https://skillicons.dev/icons?i=py,cpp&theme=dark" />

**ML / DL**

<img src="https://skillicons.dev/icons?i=pytorch,tensorflow,opencv,sklearn&theme=dark" />

**Frameworks & Tools**

<img src="https://skillicons.dev/icons?i=fastapi,supabase,docker,git,github&theme=dark" />

**Cloud & Data**

<img src="https://skillicons.dev/icons?i=aws,gcp,postgres,redis,sqlite&theme=dark" />

</div>

---

## 🚀 Featured Projects

| Project | Description |
|---------|-------------|
| [📓 book-notes-mcp](https://github.com/ayush1298/book-notes-mcp) | Multimodal PWA — turns raw text, audio & image notes into a semantic knowledge base via pgvector + MCP |
| [⚡ AI-Simulator](https://github.com/ayush1298/AI-Simulator) | Multi-agent platform generating physics simulations from natural language using 15+ LLMs |
| [📝 ML-Interview-Based-Blogs](https://github.com/ayush1298/ML-Interview-Based-Blogs) | ML concept deep-dives written in interview-prep format — fundamentals to advanced topics |
| [🎯 HiringSight](https://github.com/ayush1298/HiringSight-AI-Powered-Hiring-Intelligence-Platform) | AI-powered hiring intelligence platform for smart candidate evaluation |

---

## 📊 GitHub Activity

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=ayush1298&theme=midnight-purple&show_icons=true&include_all_commits=true&count_private=true&cache_seconds=21600" alt="GitHub Stats" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ayush1298&layout=compact&theme=midnight-purple&cache_seconds=21600" alt="Top Languages" />
  <img src="https://streak-stats.demolab.com?user=ayush1298&theme=midnight-purple&cache_seconds=21600" alt="GitHub Streak" />
</p>

---

<div align="center">
  <i>Open to research collaborations · LLMs · Agentic AI · VLMs · AI Safety</i>
</div>
